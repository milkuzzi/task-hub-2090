...
class Settings(BaseSettings):
    ...
    # Максимальная длина одного сообщения чата задачи (символов).
    max_message_len: int = Field(default=4000)

    # --- Аватары профиля (§8) ---
    # Предельный размер загружаемого аватара (МБ); тип проверяется по magic bytes.
    max_avatar_mb: int = Field(default=2)
